	...
	public function initTable(){
        ...
        if (count($this->_ct_initdata)) {
            ...
            foreach ($this->_ct_initdata as $K=>$data) {
                $query_cells = array();
                $query_insert = array();
                foreach ($data as $cellName=>$cellValue) {
                    $query_cells[] = "`{$cellName}`";
                    $query_insert[] = $this->quote($cellValue);
                }
                $query_inserts[] = "(".implode(",",$query_insert).")";
            }
            $query_insert = "INSERT INTO `{$this->_database}`.`{$this->_ct_name}` (".implode(',',$query_cells).") VALUES ".implode(',',$query_inserts);
            $this->_db->query($query_insert);
